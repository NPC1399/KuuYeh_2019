...
class Matrix:
    ...
    def font_init(self):
        self.fr = font.FontRenderer(self.width, self.height, self.pixel)
        self.fr.init()

    def print_num(self,data_num,intensity):
        self.fill(0)
    ...
    def print_text(self,message,intensity,speed):
        self.fill(0)
        Condition = 1
        pos = self.width                 # X position of the message start.
        message_width = self.fr.width(message)   # Message width in pixels.
        ...
        while Condition:
            ...
            if Condition != 0:
                ...
                frame = (frame + 1) % 2
                self.frame(frame, show=False)
                # Clear the frame and draw the text at the current position.
                self.fill(0)
                self.fr.text(message, int(pos), 0, intensity)
